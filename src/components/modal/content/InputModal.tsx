...
const InputModalContent = (props: { context: Context; actions: Actions }) => {
  ...
  const startNumberStep = amount === 0 || !amountIsFixed;
  const [activeStep, setActiveStep] = useState(
    startNumberStep ? "number" : "note"
  );
      ...
      <ModalContent>
        ...
        <ModalBody>
          {noteIsFixed ? (
            ...
            <>
              <Row>
                <H2 style={{ marginRight: "10px" }}>$</H2>
                <LargeText>{amountString}</LargeText>
              </Row>
              <H2>Add a note</H2>
              <TextArea
                value={note}
        ...
        <ModalActions>
          ...
          <ActionButton
            tone="light"
            onClick={
              amountIsFixed ? actions.reset : () => setActiveStep("number")
            }
          >
